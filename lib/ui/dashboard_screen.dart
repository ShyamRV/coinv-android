      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView(
            ...
            children: [
              _Metric('Focus', '${state.metrics.focus}%'),
              _Metric('Energy', '${state.metrics.energy}%'),
              _Metric('Learning', '${state.metrics.learningVelocity}%'),
              _Metric('Decisions', '${state.metrics.decisionReadiness}%'),
              _Metric('Memory', '${state.metrics.memoryActivity}%'),
            ],
          ),
        ),
        ...
        CoinCard(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(state.personalization.confidenceLabel),
              const SizedBox(height: 8),
              LinearProgressIndicator(
                value: state.personalization.progress / 100,
              ),
              const SizedBox(height: 4),
              Text('${state.personalization.progress}% calibrated'),
            ],
          ),
        ),
        if (state.recommendations.isNotEmpty) ...[
          const SectionTitle('Recommended next'),
          ...state.recommendations.map(
            (item) => ListTile(
              leading: const Icon(Icons.auto_awesome, color: CoinColors.blue),
              title: Text(item.text),
              subtitle: Text(item.category),
              onTap: item.route == null
                  ? null
                  : () {
                      const routes = {'voice': 1, 'memory': 2, 'decisions': 3};
                      final tab = routes[item.route];
        ...
        if (state.insights.isNotEmpty) ...[
          ...
          ...state.insights
              ...
              .map(
                (insight) => Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: CoinCard(child: Text(insight.text)),
                ),
              ),
        ],
