...
class CoinVApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      themeMode: switch (state.settings.theme) {
...
class _CoinVHomeState extends State<CoinVHome> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      unawaited(context.read<CoinVController>().handleAppResume());
    }
  }
  ...
  void _changeTab(int value) {
    setState(() => _tab = value);
    const routes = ['dashboard', 'voice', 'memory', 'decisions', 'profile'];
    unawaited(context.read<CoinVController>().recordNavigation(routes[value]));
  }
}
